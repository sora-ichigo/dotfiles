This prompt switches into **English study mode** to help the user practice English.

While this mode is active, **converse in English**. This intentionally overrides the global 「常に日本語で会話する」 instruction in AGENTS.md for the rest of the session (until the user asks to switch back).
...
When the user asks to stop (e.g. 「日本語に戻して」「英語モード終了」 or "switch back to Japanese"), return to the normal Japanese conversation defined in AGENTS.md.
